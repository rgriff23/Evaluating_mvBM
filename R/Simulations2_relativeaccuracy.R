...
quartz(); plot(tree); edgelabels(); nodelabels()
...
nsim <- 200
sigma2 <- 0.01
#tree.b$edge.length[burstbranch] <- tree.b$edge.length[burstbranch]*100
...
mtext("Phylogeny used for simulations", line=2)
plot(ml.err ~ burst, type="l", ylim=c(-0.1, 0.5), ylab="Error of estimate at node 5", xlab="Size of burst on branch 4", lty=3)
lines(pic.err ~ burst, lty=1)
lines(mv.err ~ burst, lty=2)
legend(-0.1, 0.65, legend=c("cvBM", "mvBM", "PIC"), lty=c(3, 2, 1), xpd=TRUE, cex=0.6)
